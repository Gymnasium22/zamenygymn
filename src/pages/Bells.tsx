...
const ShiftTimeline = ({
    ...
    bulkShiftSchedule: (shift: Shift, minutes: number) => void;
                ...
                {periods.map((period, index) => {
                    ...
                    return (
                        <React.Fragment key={period}>
                            ...
                            <div
                                className={`relative z-10 flex items-center gap-3 p-3 rounded-xl border-2 transition-all ${
                                    bell.cancelled
                            ...
                            >
                                <div
                                    className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-lg shadow-sm border-2 ${
                                        bell.cancelled
                                            ? 'bg-white text-red-400 border-red-100'
                                            : 'bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-600 dark:to-slate-700 text-slate-600 dark:text-slate-300 border-white dark:border-slate-500'
                                    }`}
                                >
                                    {period}
                                </div>
                                ...
                                {bell.cancelled ? (
                                    ...
                                    <>
                                        ...
                                        <button
                                            onClick={() => toggleBellCancellation(shift, period)}
                                            className="p-2 text-slate-300 hover:text-red-500 hover:bg-slate-50 dark:hover:bg-slate-600 rounded-lg transition"
                                        >
                                            <Icon name="X" size={16} />
                                        </button>
                                    </>
...
export const BellsPage = () => {
    ...
    const handleBellChange = (shift: string, period: number, field: 'start' | 'end', value: string) => {
        ...
        if (field === 'start') {
            ...
            const newEndMin = newStartMin + (duration > 0 ? duration : 45);
        ...
        <div className="max-w-7xl mx-auto w-full h-full flex flex-col">
            <div className="flex flex-wrap items-center justify-between gap-4 mb-8 shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-slate-800 dark:text-white">Режимы звонков</h1>
                        <p className="text-xs text-slate-500 dark:text-slate-400">
                            Управление расписанием звонков и экспорт документов
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto pb-20 custom-scrollbar pr-2">
                <div className="flex flex-col gap-6">
                    ...
                    <div className="lg:hidden flex bg-slate-100 dark:bg-slate-700 p-1 rounded-xl">
                        <button
                            onClick={() => setActiveBellShift(Shift.First)}
                            className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${
                                activeBellShift === Shift.First
                        ...
                        <button
                            onClick={() => setActiveBellShift(Shift.Second)}
                            className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${
                                activeBellShift === Shift.Second
                                    ? 'bg-white dark:bg-slate-600 shadow text-indigo-600 dark:text-white'
                                    : 'text-slate-500 dark:text-slate-400'
                            }`}
                        >
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                        <div className={`${activeBellShift === Shift.First ? 'block' : 'hidden lg:block'}`}>
                            <ShiftTimeline
                                shift={Shift.First}
                                currentPresetBells={currentPresetBells}
                                bulkShiftSchedule={bulkShiftSchedule}
            ...
            <Modal isOpen={isPresetModalOpen} onClose={() => setIsPresetModalOpen(false)} title="Новый режим звонков">
                <div className="space-y-4">
                    ...
                    <div className="flex justify-end pt-2">
                        <button
                            onClick={confirmCreatePreset}
                            className="px-6 py-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 text-sm font-bold"
                        >
                            Создать
                        </button>
                    </div>
            ...
            <Modal
                ...
                title="Экспорт расписания звонков"
                maxWidth="max-w-6xl"
            >
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-slate-50 dark:bg-slate-700/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-600">
                        ...
                        <div className="flex gap-2">
                            ...
                            <button
                                ...
                                className="px-4 py-2 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 transition flex items-center gap-2 text-sm shadow-lg shadow-red-200 dark:shadow-none"
                            >
                                <Icon name="FileSpreadsheet" size={18} /> Скачать PDF
                            </button>
                            <button
                                onClick={exportBellsToExcel}
                                className="px-4 py-2 bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 transition flex items-center gap-2 text-sm shadow-lg shadow-green-200 dark:shadow-none"
                            >
