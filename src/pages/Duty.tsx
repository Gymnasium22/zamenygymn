...
export const DutyPage = () => {
    ...
    const getConflict = (zoneId: string, day: string, shift: string, teacherId?: string) => {
        ...
        const conflict = dutySchedule.find(
            (d) => d.teacherId === teacherId && d.day === day && d.shift === shift && d.zoneId !== zoneId
        );
        return !!conflict;
    };
        ...
        for (const day of DAYS) {
            for (const shift of shifts) {
                const candidates = teachers.filter((t) => {
                    const lessonsCount = activeSchedule.filter(
                        (s) => s.teacherId === t.id && s.day === day && s.shift === shift
                    ).length;
                ...
                for (const zone of dutyZones) {
                    const scoredCandidates = candidates.map((t) => {
                        ...
                        const lessonsInZone = activeSchedule.filter(
                            (s) =>
                                s.teacherId === t.id &&
                                s.day === day &&
                                s.shift === shift &&
                        ...
                        const totalLessons = activeSchedule.filter(
                            (s) => s.teacherId === t.id && s.day === day && s.shift === shift
                        ).length;
    ...
    const handleZoneSave = async () => {
        ...
        const roomsArray = zoneStringInput
            .split(',')
            .map((s) => s.trim())
            .filter(Boolean);
    ...
    const getRecommendations = () => {
        ...
        const candidates = teachers.map((t) => {
            const lessonsCount = activeSchedule.filter(
                (s) => s.teacherId === t.id && s.day === day && s.shift === selectedShift
            ).length;
            const isBusy = dutySchedule.some(
                (d) =>
                    d.day === day &&
                    d.teacherId === t.id &&
                    d.zoneId !== selectedCell.zoneId &&
                    d.shift === selectedShift
            );

            const lessonsInZone =
                zone && zone.includedRooms
        ...
        const others = candidates
            ...
            .map((c) => ({
                value: c.t.id,
                label: c.t.name + (c.isBusy ? ' (Занят)' : '') + (c.lessonsInZone === 0 ? ' (Нет уроков в зоне)' : '')
            }));
        ...
        <div className="max-w-7xl mx-auto w-full pb-20">
                        ...
                        <thead>
                            <tr className="bg-slate-50 dark:bg-slate-700/50">
                                ...
                                {DAYS.map((day) => (
                                    <th
                                        key={day}
                                        className="p-3 text-center text-sm font-bold text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-700 min-w-[120px]"
                                    >
                                        {day}
                                    </th>
                                ))}
                                    ...
                                    {DAYS.map((day) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                {teacher ? (
                                                    <div
                                                        className={`font-bold text-sm ${hasConflict ? 'text-red-600 dark:text-red-400' : 'text-indigo-700 dark:text-indigo-400'}`}
                                                    >
                                                        {teacher.name}
                                                        {hasConflict && (
                                                            <div className="text-[9px] uppercase">Конфликт</div>
                                                        )}
                                                    </div>
                                                ) : (
                                                    <div className="text-slate-300 dark:text-slate-600 text-lg opacity-0 group-hover:opacity-100">
                                                        +
                                                    </div>
                                                )}
                                            </td>
            ...
            <div className="mt-8 bg-slate-100 dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700">
                ...
                <div className="overflow-auto custom-scrollbar">
                    <div
                        ...
                        className="bg-white p-12 min-w-[1200px] text-black font-serif shadow-lg"
                    >
                        <div
                            className="flex justify-end items-start mb-8 text-sm leading-snug"
                            style={{ marginTop: manualOffsets.approvalBlock }}
                        >
                            <div className="w-[350px]" contentEditable suppressContentEditableWarning>
                                ...
                                <div className="mt-8 flex items-end">
                                    <div
                                        className="border-b border-black flex-grow h-4"
                                        style={{ marginBottom: manualOffsets.signatureLine }}
                                    ></div>
                        ...
                        <div
                            ...
                            style={{ marginTop: manualOffsets.mainTitle }}
                        >
                            <h1 className="text-xl font-bold uppercase">ГРАФИК ДЕЖУРСТВА</h1>
                            <h2 className="text-lg">
                                учителей по государственному учреждению образования «Гимназия № 22 г. Минска»
                            </h2>
                            <h3 className="text-lg font-bold mt-1">
                                на {semester}-е полугодие {new Date().getFullYear()}/{new Date().getFullYear() + 1}{' '}
                                учебного года
                            </h3>
                        </div>
                                ...
                                <table className="w-full border-collapse border border-black text-sm text-center">
                                    <thead>
                                        <tr>
                                            <th
                                                rowSpan={2}
                                                className="border border-black p-1 bg-gray-100 w-32 font-bold align-middle"
                                                contentEditable
                                                suppressContentEditableWarning
                                                style={{ paddingTop: manualOffsets.tableHeaders }}
                                            >
                                                День недели
                                            </th>
                                            {zonesByFloor.map((group) => (
                                                <th
                                                    key={group.floor}
                                                    colSpan={group.zones.length}
                                                    className="border border-black p-1 bg-gray-200 font-bold uppercase text-xs align-middle"
                                                    contentEditable
                                                    suppressContentEditableWarning
                                                    style={{ paddingTop: manualOffsets.tableHeaders }}
                                                >
                                        ...
                                        <tr>
                                            {allZonesSorted.map((zone) => (
                                                <th
                                                    key={zone.id}
                                                    className="border border-black p-1 bg-gray-100 font-bold align-middle"
                                                    contentEditable
                                                    suppressContentEditableWarning
                                                    style={{ paddingTop: manualOffsets.tableHeaders }}
                                                >
                                    ...
                                    <tbody>
                                        {DAYS.map((day) => (
                                            <tr key={day}>
                                                <th
                                                    className="border border-black p-2 font-bold bg-gray-50 text-left align-middle"
                                                    contentEditable
                                                    suppressContentEditableWarning
                                                >
                                                    {day}
                        ...
                        <div
                            className="mt-8 flex items-end justify-between text-sm font-bold"
                            contentEditable
                            suppressContentEditableWarning
                            style={{ marginTop: manualOffsets.footerBlock }}
                        >
                            <div>Секретарь учебной части гимназии №22 г.Минска</div>
                            <div
                                className="border-b border-black flex-grow mx-4 mb-1"
                                style={{ marginBottom: manualOffsets.signatureLine }}
                            ></div>
                            <div>Е.К.Шунто</div>
                        </div>
            ...
            >
                <div className="space-y-6 min-h-[550px] flex flex-col">
                    {selectedCell && (
                        <div className="bg-slate-50 dark:bg-slate-700/50 p-4 rounded-2xl mb-2 text-base border border-slate-100 dark:border-slate-600 shadow-sm">
                            <span className="font-bold text-slate-800 dark:text-white text-lg">
                                {dutyZones.find((z) => z.id === selectedCell.zoneId)?.name}
                            </span>
                    ...
                    <div>
                        <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-2 tracking-wider">
                            Учитель
                        </label>
                        <p className="text-[10px] text-slate-400 mb-2">
                            Можно назначать учителей даже без уроков в этот день
                        </p>
                        <SearchableSelect
                            options={[
                    ...
                    <div className="flex justify-end gap-3 pt-4 mt-auto">
                        <button
                            onClick={handleRemoveDuty}
                            className="px-5 py-3 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl font-bold text-sm transition-colors"
                        >
                            Снять дежурство
                        </button>
                        <button
                            onClick={handleTeacherSelect}
                            className="px-8 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition shadow-lg shadow-indigo-200 dark:shadow-none"
                        >
                            Сохранить
                        </button>
                    </div>
            ...
            <Modal
                ...
                title={editingZone.id ? 'Редактировать зону' : 'Добавить зону'}
            >
