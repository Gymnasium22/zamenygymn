    ...
    const freeTeachers = useMemo(() => {
        ...
        return teachers
            .filter((teacher) => {
                ...
                const hasLateLessons = activeSchedule.some(
                    (s) =>
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-6">
                    ...
                    <div className="flex gap-2 bg-slate-100 dark:bg-slate-700 p-1 rounded-xl">
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'teachers' ? 'bg-white dark:bg-slate-600 shadow text-indigo-600 dark:text-white' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700'}`}
                        >
                            Учителя
                        </button>
                        <button
                            onClick={() => setActiveTab('rooms')}
                            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'rooms' ? 'bg-white dark:bg-slate-600 shadow text-indigo-600 dark:text-white' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700'}`}
                        >
                            Кабинеты
                        </button>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-8 items-end">
                        <div>
                            ...
                            <div className="flex gap-1 bg-slate-100 dark:bg-slate-700 p-1 rounded-xl">
                                <button
                                    onClick={() => setTeacherShift(Shift.First)}
                                    className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${teacherShift === Shift.First ? 'bg-white dark:bg-slate-600 shadow text-indigo-600 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}
                                >
                                    1 смена
                                </button>
                                <button
                                    onClick={() => setTeacherShift(Shift.Second)}
                                    className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${teacherShift === Shift.Second ? 'bg-white dark:bg-slate-600 shadow text-indigo-600 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}
                                >
                                    2 смена
                                </button>
                            </div>
                ...
                {activeTab === 'rooms' && (
                    <div className="flex flex-wrap gap-8 items-end">
                        <div>
                            ...
                            <div className="flex gap-1 bg-slate-100 dark:bg-slate-700 p-1 rounded-xl">
                                <button
                                    onClick={() => setRoomShift(Shift.First)}
                                    className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${roomShift === Shift.First ? 'bg-white dark:bg-slate-600 shadow text-indigo-600 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}
                                >
                                    1 смена
                                </button>
                                <button
                                    onClick={() => setRoomShift(Shift.Second)}
                                    className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${roomShift === Shift.Second ? 'bg-white dark:bg-slate-600 shadow text-indigo-600 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}
                                >
                                    2 смена
                                </button>
                            </div>
            ...
            {activeTab === 'teachers' && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {freeTeachers.map((t) => (
                        <div
                            key={t.id}
                            className="bg-white dark:bg-dark-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm flex items-center gap-4 hover:shadow-md transition-all"
                        >
                            ...
                            <div className="min-w-0">
                                <div className="font-bold text-slate-800 dark:text-slate-200 text-lg leading-tight truncate">
                                    {t.name}
                                </div>
                                <div className="flex flex-wrap gap-1 mt-1.5">
                                    {t.subjectIds.slice(0, 2).map((sid) => {
                                        ...
                                        return s ? (
                                            <span
                                                key={sid}
                                                className="text-[10px] px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 font-medium"
                                            >
                                                {s.name}
                                            </span>
                                        ) : null;
                    ...
                    {freeTeachers.length === 0 && (
                        <div className="col-span-full py-12 text-center text-slate-400">
                            Не найдено учителей, свободных после {afterPeriod} урока в {teacherShift}
                        </div>
                    )}
                </div>
            )}
            ...
            {activeTab === 'rooms' && (
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                    {freeRooms.map((roomName) => (
                        <div
                            key={roomName}
                            className="bg-white dark:bg-dark-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm text-center flex flex-col items-center justify-center hover:border-indigo-500 transition-all hover:shadow-md group"
                        >
