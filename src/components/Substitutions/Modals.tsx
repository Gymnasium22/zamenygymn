    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Причина отсутствия">
        <div className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
                 <button onClick={onConfirm} className="w-full px-6 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition">
                     Только отметить отсутствие
                 </button>
                 <button onClick={onOpenBatch} className="w-full px-6 py-3 bg-purple-50 text-purple-700 border border-purple-100 rounded-xl font-bold hover:bg-purple-100 transition flex items-center justify-center gap-2">
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Пакетная обработка">
        <div className="space-y-6">
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-700 flex justify-end gap-3">
                <button onClick={onClose} className="px-4 py-2 text-slate-500 font-bold hover:bg-slate-100 rounded-lg">Отмена</button>
                <button 
                    onClick={onConfirm} 
...
export const ManualSearchModal: React.FC<ManualSearchModalProps> = ({
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Ручной поиск урока">
        ...
        <div className="space-y-2 max-h-64 overflow-y-auto custom-scrollbar">
            ...
            {searchValue.length > 1 && results.length === 0 && <div className="text-center text-slate-400 text-xs py-4">Уроки не найдены на выбранную дату ({selectedDate})</div>}
        </div>
    </Modal>
);
...
export const TelegramChoiceModal: React.FC<TelegramChoiceModalProps> = ({
    isOpen,
    onClose,
    onConfirm
}) => (
...
export const QuickViewScheduleModal: React.FC<QuickViewScheduleModalProps> = ({
    ...
    <Modal isOpen={!!teacherId} onClose={onClose} title="Расписание учителя" maxWidth="max-w-md">
         <div className="space-y-3">
             {teacherId && (() => {
                 const schedule = activeSchedule.filter(s => s.teacherId === teacherId && s.day === selectedDayOfWeek).sort((a,b) => a.period - b.period);
