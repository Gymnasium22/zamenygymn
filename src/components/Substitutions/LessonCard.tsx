...
export const LessonCard: React.FC<LessonCardProps> = ({
    ...
    const rep =
        firstSub && !['conducted', 'cancelled'].includes(firstSub.replacementTeacherId)
            ? teachers.find((t) => t.id === firstSub.replacementTeacherId)
            : null;
    ...
    const replacementRoomName = replacementRoomId
        ? rooms.find((r) => r.id === replacementRoomId)?.name || replacementRoomId
        : null;
    ...
    if (isCompactMode) {
        return (
            ...
            >
                ...
                <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-sm ${isResolved ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}
                >
                    {l.period}
                </div>
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 text-xs">
                        <span className={`font-medium ${isResolved ? 'text-slate-400 line-through' : 'text-red-500'}`}>
                            {orig?.name}
                        </span>
                        {isResolved && <Icon name="ArrowRight" size={10} className="text-slate-300" />}
                        {isResolved && (
                            <span
                                className={`font-bold ${isCancelled ? 'text-red-600' : isConducted ? 'text-blue-600' : 'text-emerald-600'}`}
                            >
                                {isCancelled ? 'СНЯТ' : isConducted ? 'ПРОВЕДЕН' : rep?.name}
                            </span>
                        )}
                    </div>
                    {comment && <div className="text-[10px] text-slate-500 italic mt-0.5 line-clamp-2">{comment}</div>}
                </div>
                ...
                <div className="flex items-center gap-2">
                    {isRoomChanged ? (
                        ...
                        <div className="text-[10px] bg-slate-100 px-1.5 py-0.5 rounded font-bold text-slate-500">
                            {originalRoomName}
                        </div>
                    )}
        ...
        >
            ...
            <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-4">
                    <div
                        className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-xl ${isResolved ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}
                    >
                        {l.period}
                    </div>
                    <div>
                ...
                <div className="text-right">
                    <div className="text-xs font-bold text-slate-400 uppercase">
                        {l.shift === Shift.First ? '1 смена' : '2 смена'}
                    </div>
                    <div className="mt-1">
                        {isRoomChanged ? (
                            <div className="flex items-center justify-end gap-1.5 bg-indigo-50 dark:bg-indigo-900/30 px-2 py-1 rounded border border-indigo-100 dark:border-indigo-800">
                                <span className="text-xs font-mono font-bold text-slate-400 line-through decoration-red-400">
                                    {originalRoomName}
                                </span>
                            ...
                            <div className="text-xs font-mono bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded inline-block font-bold">
                                {originalRoomName}
                            </div>
                        )}
                    </div>
            ...
            <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-100 dark:border-slate-700">
                ...
                {!isResolved ? (
                    ...
                    <div className="flex items-center gap-3 justify-end">
                        {isCancelled ? (
                            <span className="text-red-600 font-black text-sm uppercase">УРОК СНЯТ</span>
                        ) : isConducted ? (
                            <span className="text-blue-600 font-bold text-sm">ПРОВЕДЕН</span>
                        ) : (
                            <div className="flex flex-col items-end">
                                <span className="text-emerald-600 font-bold text-base">{rep?.name || 'Замена'}</span>
                                {firstSub?.isMerger && (
                                    <span className="text-[10px] text-purple-500 font-bold">ОБЪЕДИНЕНИЕ</span>
                                )}
                            </div>
                        )}
