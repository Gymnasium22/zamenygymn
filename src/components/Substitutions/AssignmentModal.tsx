            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6 bg-white dark:bg-dark-800">
                                    ...
                                    {candidates.recommended.map(({ teacher, isSpecialist }) => (
                                        <div key={teacher.id} className="flex items-center justify-between p-3 bg-emerald-50/50 border border-emerald-100 rounded-xl hover:bg-emerald-100/50 transition-colors">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <button onClick={() => onViewSchedule(teacher.id)} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-white rounded-lg" title="Посмотреть расписание"><Icon name="Eye" size={16}/></button>
                                                <button onClick={() => onAssign(teacher.id)} className="px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-bold hover:bg-emerald-700">Выбрать</button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                                    ...
                                    return (
                                        <div key={teacher.id} className={`flex items-center justify-between p-3 rounded-lg text-sm ${isAbsent ? 'opacity-70 bg-red-50/50' : 'hover:bg-slate-50'}`}>
                                            ...
                                            <div className="flex gap-2">
                                                <button onClick={() => onViewSchedule(teacher.id)} className="p-1.5 text-slate-300 hover:text-indigo-600 hover:bg-slate-100 rounded" title="Посмотреть расписание"><Icon name="Eye" size={16}/></button>
                                                <button onClick={(e) => onToggleRefusal(teacher.id, e)} className="p-1.5 text-slate-300 hover:text-slate-500 hover:bg-slate-100 rounded"><Icon name="X" size={16}/></button>
                                                <button 
                ...
                {subMode === 'cancel' && (
                    <div className="text-center py-12 flex flex-col items-center justify-center h-full">
                        ...
                        <button onClick={() => onAssign('cancelled')} className="px-8 py-4 bg-red-600 text-white rounded-xl font-bold text-lg hover:bg-red-700 shadow-lg shadow-red-200 hover:shadow-xl transition-all">Подтвердить отмену</button>
                    </div>
                )}

                {subMode === 'advanced' && (
                    <div className="space-y-6 pt-4">
                        <button onClick={() => onAssign('conducted')} className="w-full p-4 rounded-xl bg-blue-50 text-blue-700 font-bold text-base hover:bg-blue-100 flex items-center justify-center gap-3 border border-blue-100 transition-all hover:shadow-md"><Icon name="CheckCircle" size={20}/> Урок проведен (без замены)</button>
                        ...
                        <div className="bg-amber-50 p-5 rounded-2xl border border-amber-100">
                            ...
                            <div className="space-y-2">
                                {mergeCandidates.length === 0 ? <div className="text-sm text-amber-500 italic text-center py-4">Нет подходящих классов</div> : mergeCandidates.map(c => (
                                    <button key={c.classEntity.id} onClick={() => onBatchClassMerge(c.classEntity.id)} className="w-full p-3 bg-white rounded-xl border border-amber-200 text-left text-sm hover:border-amber-400 hover:shadow-sm transition-all">
                            ...
                            <div className="bg-purple-50 p-5 rounded-2xl border border-purple-100">
                                ...
                                <div className="space-y-2">
                                    {otherLessonsForTeacher.map(l => (
                                        <button key={l.id} onClick={() => onSwapLessons(l.id)} className="w-full p-3 bg-white rounded-xl border border-purple-200 text-left text-sm hover:border-purple-400 hover:shadow-sm transition-all">
