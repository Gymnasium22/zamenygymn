...
export function SanitaryScheduleTab(props: {
    ...
    const selectedClass = useMemo(
        () => eligibleClasses.find((c) => c.id === selectedClassId),
    ...
    const [manualSwaps, setManualSwaps] = useState<Array<{ classId: string; shift: string; from: Slot; to: Slot }>>([]);
    ...
    const allClassesStats = useMemo(() => {
        const rows = eligibleClasses.map((c) => {
            ...
            const violationTypes = {
                heavyFirstOrLast:
                    a?.violations.filter((v) => v.type === 'heavy_first_or_last_more_than_once').length ?? 0,
    ...
    const sanitarySchedule = useMemo(() => {
        ...
        const relevantSwaps = manualSwaps.filter(
            (s) => s.classId === selectedClass.id && s.shift === selectedClass.shift
        );
            ...
            return items
                .map((it) => {
                    ...
                    const roomPart = roomName
                        ? ` <span style="font-size:10px; font-weight:bold;">каб. ${roomName}</span>`
                        : '';
    ...
    const downloadAllClassesXls = () => {
        ...
        const getLessonDisplayName = (subjectName: string, direction?: string): string => {
            if (isPELesson(subjectName)) {
                ...
                if (
                    (direction && direction.toLowerCase().includes('игров')) ||
                    subjectName.toLowerCase().includes('игров')
                ) {
        ...
        shifts.forEach((shift) => {
            ...
            DAYS.forEach((day) => {
                ...
                shiftClasses.forEach((cls) => {
                    ...
                    const totalDifficulty = dayLessons.reduce(
                        (sum, lesson) => sum + getSubjectDifficulty(lesson.subjectId),
                        0
                    );
            ...
            shiftClasses.forEach((cls) => {
                const weekLessons = currentSchedule.filter((s) => s.classId === cls.id && s.shift === shift);
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                    <div>
                        <h2 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
                            <Icon name="Shield" className="text-indigo-600 dark:text-indigo-400" /> Санстанция
                            (недельный шаблон)
                        </h2>
                        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                            Автонастройка распределяет нагрузку по требованиям, не меняя действующее расписание. Можно
                            вручную обменять ячейки и экспортировать плакат.
                        </p>
                        <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
                            {isOptimizing ? (
                                ...
                                <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 font-bold">
                                    {lastOptimizedAt ? (
                                        <>
                                            Обновлено: <span className="font-black">{lastOptimizedAt}</span>
                                        </>
                                    ) : (
                                        'Готово'
                                    )}
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        >
                            {isExporting ? (
                                <>
                                    <Icon name="Loader" size={18} className="animate-spin" /> Создание...
                                </>
                            ) : (
                                <>
                                    <Icon name="Download" size={18} /> Скачать плакат
                                </>
                            )}
                        </button>
                        <button
                    ...
                    <div className="rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                        <table className="w-full text-sm">
                            <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                                <tr>
                                    <th className="text-left p-3 font-black text-xs uppercase tracking-wider text-slate-500">
                                        Класс
                                    </th>
                                    <th className="text-left p-3 font-black text-xs uppercase tracking-wider text-slate-500">
                                        Смена
                                                        ...
                                                        {(a?.violations || []).slice(0, 6).map((v, idx) => (
                                                            <li key={idx}>
                                                                {v.type === 'heavy_first_or_last_more_than_once' && (
                                                                    <>
                                                                        Тяжёлые на{' '}
                                                                        {v.position === 'first' ? '1-м' : 'последнем'}:{' '}
                                                                        {v.count} раз (норма ≤ 1)
                                                                    </>
                                                                )}
                                                                {v.type === 'heavy_consecutive' && (
                                                                    <>
                                                                        Тяжёлые подряд: {v.day} (около {v.period}-го)
                                                                    </>
                                                                )}
                                                                {v.type === 'peak_day_not_on_recommended' && (
                                                                    <>
                                                                        Пик нагрузки на {v.peakDay} (рекомендуется
                                                                        Вт/Ср/Пт)
                                                                    </>
                                                                )}
                                                            </li>
                        ...
                        <div>
                            ...
                            <div className="mt-2 text-xs font-bold text-slate-500 dark:text-slate-400">
                                ...
                                <span className="font-black">
                                    {modeLabel === 'auto' ? 'Автонастройка' : 'Как в действующем расписании'}
                                </span>
                            </div>
                        </div>
                    ...
                    <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div className="p-6 rounded-2xl bg-slate-50 dark:bg-slate-900/30 border border-slate-200 dark:border-slate-700">
                            ...
                            <div className="text-sm font-bold text-slate-700 dark:text-slate-200 mt-2 space-y-2">
                                <div>
                                    1) Жми <span className="font-black">Автонастройка</span> → смотри, где остались
                                    замечания.
                                </div>
                                <div>2) Открой класс → сделай 1–2 обмена вокруг тяжёлых предметов.</div>
                                <div>
                                    3) Экспортируй <span className="font-black">XLS (все классы)</span>.
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <table className="w-full text-sm">
                            <thead className="sticky top-0 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                                <tr>
                                    ...
                                    <th className="text-right p-3 font-black text-xs uppercase tracking-wider text-slate-500">
                                        Замечания
                                    </th>
                                </tr>
                            </thead>
                        ...
                        <div ref={posterRef} className="bg-white text-slate-900 p-6 rounded-xl border border-slate-200">
                            <div className="flex items-end justify-between gap-4 border-b-2 border-slate-800 pb-3 mb-4">
                                ...
                                <div className="text-right">
                                    ...
                                    <div className="text-sm font-bold">
                                        {manualSwaps.length ? `Правки: ${manualSwaps.length}` : `Авто`}
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            {DAYS.map((d) => (
                                                <th
                                                    key={String(d)}
                                                    className="border-2 border-black bg-slate-100 py-2 font-black uppercase tracking-wider"
                                                >
                                                    {d}
                                                </th>
                                            ))}
                                        ...
                                        {periods.map((p) => (
                                            <tr key={String(p)}>
                                                ...
                                                {DAYS.map((day) => {
                                                    ...
                                                    const isPicked =
                                                        activePick && activePick.day === day && activePick.period === p;
                                                    const hasChanges = manualSwaps.some(
                                                        (s) =>
                                                            (s.from.day === day && s.from.period === p) ||
                                                            (s.to.day === day && s.to.period === p)
                                                    );
                                                    return (
                                                        <td
                                                            key={String(k)}
                                                            ...
                                                            className={[
                                                                ...
                                                                isPicked
                                                                    ? 'ring-4 ring-indigo-400 ring-inset bg-indigo-50'
                                                                    : 'hover:bg-slate-50',
                            ...
                            <div className="mt-4 pt-3 border-t border-slate-200 flex items-center justify-between text-[10px] text-slate-500">
                                ...
                                <div className="font-mono">
                                    swaps:auto {sanitaryBase.swapsApplied} • manual {manualSwaps.length}
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        <div className="bg-white dark:bg-dark-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
                            ...
                            <div className="mt-3 text-xs text-slate-500 dark:text-slate-400">
                                Автонастройка всегда выполняется сразу для всей недели и всех классов. Выбор класса
                                влияет только на просмотр и ручные правки.
                            </div>
                        ...
                        <div className="bg-white dark:bg-dark-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
                            ...
                            {!isVtoXI && (
                                <div className="mt-3 text-xs font-bold text-slate-500 bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 rounded-xl p-3">
                                    Для классов ниже V требования по “тяжёлым предметам” не применяются в полном объёме
                                    (по текущей логике проверки).
                                </div>
                            )}
                            ...
                            {analysis && (
                                <div className="mt-4 space-y-4">
                                    <div className="grid grid-cols-2 gap-3">
                                        ...
                                        <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600">
                                            <div className="text-[10px] font-black text-slate-400 uppercase tracking-wider">
                                                Тяжёлые на последнем
                                            </div>
                                            <div className="text-2xl font-black text-slate-800 dark:text-white">
                                                {analysis.heavyLastCount}
                                            </div>
                                            <div className="text-[10px] font-bold text-slate-500">
                                                Норма: ≤ 1 / нед.
                                            </div>
                                            ...
                                            {DAYS.map((d) => {
                                                ...
                                                return (
                                                    <div key={String(d)} className="flex items-center gap-3">
                                                        <div className="w-10 text-xs font-black text-slate-600 dark:text-slate-300">
                                                            {d}
                                                        </div>
                                    ...
                                    {!okViolations && (
                                        <div className="p-4 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900">
                                            <div className="text-xs font-black text-red-700 dark:text-red-300 uppercase tracking-wider mb-2">
                                                Замечания
                                            </div>
                                                ...
                                                {analysis.violations.slice(0, 8).map((viol, idx) => (
                                                    <li key={idx}>
                                                        {viol.type === 'heavy_first_or_last_more_than_once' && (
                                                            <>
                                                                Тяжёлые на{' '}
                                                                {viol.position === 'first' ? '1-м' : 'последнем'} уроке:{' '}
                                                                {viol.count} раз (норма ≤ 1)
                                                            </>
                                                        )}
                                            ...
                                            {analysis.violations.length > 8 && (
                                                <div className="mt-2 text-[10px] text-red-600 dark:text-red-300">
                                                    …ещё {analysis.violations.length - 8}
                                                </div>
                                            )}
                                            <div className="mt-3 text-[10px] text-red-700/80 dark:text-red-200/80">
                                                Подсказка: попробуй “Автонастройка”, а затем 1–2 обмена ячеек вокруг
                                                тяжёлых предметов (лучше на 2–4 урок).
                                            </div>
                                        </div>
                                    )}
                                </div>
                                            ...
                                            <table className="w-full border-collapse border border-slate-200 dark:border-slate-700 text-xs">
                                                <thead>
                                                    <tr className="bg-slate-50 dark:bg-slate-800">
                                                        <th className="border border-slate-200 dark:border-slate-700 p-2 w-10">
                                                            У
                                                        </th>
                                                        {DAYS.map((d) => (
                                                            <th
                                                                key={String(d)}
                                                                className="border border-slate-200 dark:border-slate-700 p-2"
                                                            >
                                                                {d}
                                                            </th>
                                                        ))}
                                                ...
                                                <tbody>
                                                    {periods.map((p) => (
                                                        <tr key={String(p)}>
                                                            <td className="border border-slate-200 dark:border-slate-700 p-2 font-black bg-slate-50 dark:bg-slate-800">
                                                                {p}
                                                            </td>
                                                            {DAYS.map((day) => {
                                                                const k = slotId({ day, period: p });
                                                                ...
                                                                return (
                                                                    <td
                                                                        key={String(k)}
                                                                        className="border border-slate-200 dark:border-slate-700 p-2 align-top"
                                                                    >
                                                                        {items.map((it) => {
                                                                            const subj = subjectsById.get(it.subjectId);
                                                                            return (
